...
alpha = 'abcdefghijklmnopqrstuwyzABCDEFGHIJKLMNOPQRSTUWYZ1234567890' #Extra characters been added due to the password being encrypted which might consists of upper letters or even numbers
...
#We recieve details about our bank transactions through our emails
#ps i didn't type my correct gmail account password but when tested you can type your account details correctly
def sendmail(c5):
    server = smtplib.SMTP('smtp.gmail.com', 587)
    server.starttls()
    server.login('ngrinesa@gmail.com', '*****************************') #
    msg = "SUCCESSFULL TRANSACTION FROM YOUR ACCOUNT"
    server.sendmail('ngrinesa@gmail.com', c5, msg)
    server.quit()
...
def fun():
    ...
    def database():
        ...
        with conn:
            cursor = conn.cursor()
        cursor.execute(
            'CREATE TABLE IF NOT EXISTS Details1(Name TEXT,Password TEXT,Pin Number CANDIDATE KEY,Account_Num Number PRIMARY KEY,Address TEXT,Email TEXT,PhoneNumber TEXT,AlternatePhoneNumber TEXT,Branch TEXT,Country TEXT,Gender TEXT,Amount Number)')
        cursor.execute(
            'INSERT INTO Details1(Name,Password,Pin,Account_Num,Address,Email,PhoneNumber,AlternatePhoneNumber,Branch,Country,Gender,Amount) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',
            (name3, pas13, cvv, ac1, add, email1, phone3, phone4, branch1, coun, gend, amou))
    ...
    list = ["Kosovo", "England", "Australia", "USA", "India"];
    ...
    def trigger():
        ...
        if (name.get() == 'null' or passw.get() == 'null' or address.get() == 'null' or cv.get() == 'null' or ac.get() == 'null' or email.get() == 'null' or phone.get() == 'null' or phone1.get() == 'null' or branch.get() == 'null'
                or gen == 'null'):
...
def login():
    ...
    def fr():
        ...
        i = 0
        for i in range(0,
                       len(data)):  # condition to check if data is found in database and if not found displaying correct error and also asking user for new input
            if (data[i][1] == metext):
                if (decrypt(data[i][0]) == mtext):
                    ...
                    col = col + 1
                    if (col == 3):
                        tkinter.messagebox.showerror("SIGN UP!! Seems you don't have an account",
        ...
        if (v == 1 and col == 0):
                        ...
                        if (data1[i][0] == c3):
                            if (data1[i][1] == c4):
                                ...
                                if (cou.get() == "DEBIT"):
                                    ...
                                    head22 = Label(to, text="DEBIT AMOUNT", bd=2, font=("italic", 20), bg='purple',
                                                   fg='yellow').place(x=300, y=200)
                                    ...
                                    def fr3():
                                        ...
                                        if (dah < 0):
                                            tkinter.messagebox.showwarning("UNSUCCESSFUL TRANSACTION",
                                                                           "INSUFFICIENT MONEY IN ACCOUNT")

                                        else:

                                            cursor.execute('INSERT INTO Details3(Account_Num,Amount) VALUES(?,?)',
                                                           (c3, dah))
                                            cursor.execute('UPDATE Details1 SET Amount=? WHERE Account_Num=?',
                                                           (dah, c3))
                                            conn.commit()

                                            # sendmail(c5)
                                            msg = tkinter.messagebox.askyesno("Successfull Transaction",
                                                                              "To see MINI STATEMENT PRESS YES else NO")
                                            ...
                                            def passbook():
                                                ...
                                                cursor = conn.cursor()
                                                cursor.execute(
                                                    'SELECT Account_Num,Amount from Details3 WHERE Account_Num=?',
                                                    (c3,))
                                                daf = cursor.fetchall()
                                                conn.commit
                                                ...
                                                if (len(daf) < 10):
                                                    for i in range(0, len(daf)):
                                                        Label(to1, text=('Amount: {} = {}'.format(b, daf[i][1])), bd=1,
                                                              font="italic").place(x=300, y=(0 + y1))
                                                        Label(to1,
                                                              text='----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------',
                                                              bd=1, font="italic").place(x=0, y=(y1 + 20))
                                                        y1 = y1 + 40
                                                        b = b + 1
                                                else:
                                    ...
                                    submit = Button(to, text="Submit", fg="silver", bg="grey", font=("italic", 12),
                                                    command=fr3).place(x=290, y=330)
                                    ...
                                    def credit():
                                        ...
                                        def frk():
                                            ...
                                            cursor = conn.cursor()
                                            cursor.execute(
                                                'SELECT Name,Account_Num,Branch from Details1 WHERE Name=? and Account_Num=? and Branch=?',
                                                (cna, accnm, amcr1))
                                            data25 = cursor.fetchall()

                                            if (len(data25) == 0):
                                                tkinter.messagebox.showwarning("No Such Account Exists",
                                                                               "Try The Right Account")
                                                msg = tkinter.messagebox.askyesno("Want to Continue?",
                                                                                  "Try The Right Account Again")
                                                if (msg == True):
                                                    credit()
                                                ...
                                                head = Label(to16, text="CREDIT AMOUNT", bd=2, font=("italic", 20),
                                                             bg='light blue', fg='blue').place(x=300, y=200)
                                                ty = Label(to16, text="Amount", bd=2, font=("italic", 15)).place(x=260,
                                                                                                                 y=280)
                                                ...
                                                def fr3():
                                                    ...
                                                    conn = sqlite3.connect('ATM.db')
                                                    cursor = conn.cursor()
                                                    cursor.execute('SELECT Amount from Details1 WHERE Account_Num=?',
                                                                   (accnm,))
                                                    data2 = cursor.fetchall()
                                                    cursor.execute('SELECT Amount from Details1 WHERE Account_Num=?',
                                                                   (c3,))
                                                    ...
                                                    if (dah1 < 0):
                                                        ...
                                                        cursor.execute(
                                                            'INSERT INTO Details3(Account_Num,Amount) VALUES(?,?)',
                                                            (accnm, dah))
                                                        cursor.execute(
                                                            'UPDATE Details1 SET Amount=? WHERE Account_Num=?',
                                                            (dah, accnm))
                                                        cursor.execute(
                                                            'INSERT INTO Details3(Account_Num,Amount) VALUES(?,?)',
                                                            (c3, dah1))
                                                        ...
                                                        def passbook1():
                                                            ...
                                                            if (len(daf) < 10):
                                                                for i in range(0, len(daf)):
                                                                    Label(to12,
                                                                          text=('Amount: {} = {}'.format(b, daf[i][1])),
                                                                          bd=2, font="italic").place(x=300, y=(0 + y1))
                                                                    Label(to12,
                                                                          text='----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------',
                                                                          bd=1, font="italic").place(x=0, y=(y1 + 20))
                                                                    y1 = y1 + 40
                                                ...
                                                submit = Button(to16, text="Submit", fg="silver", bg="grey",
                                                                font=("italic", 15), command=fr3).place(x=300, y=330)

                                        to67 = Toplevel(height=1000, width=1000)
                                        to67.config(background='light blue')
                                        he = Label(to67, image=img6).place(x=100, y=0)
                                        he1 = Label(to67, text="THE ACCOUNT YOU WANT TO CREDIT IN ", font=("bold", 20),
                                                    bd=4, bg='yellow', fg='red').place(x=250, y=300)
                                        he1 = Label(to67, text="Name", font=("italic", 12), bd=2).place(x=300, y=400)
                                        he2 = Entry(to67, bd=4, textvar=name, font=20).place(x=502, y=402)
                                        he3 = Label(to67, text="Account Number", font=("italic", 12), bd=2).place(x=300,
                                                                                                                  y=450)
                                        ...
                                        submit = Button(to67, text="Submit", fg="silver", bg="grey",
                                                        font=("italic", 14), command=frk)
            ...
            submit = Button(fre, text="Submit", fg="white", bg="grey", font=("italic", 15), command=fr1).place(x=340,
                                                                                                               y=520)
            b1 = Button(fre, text="CREATE A NEW ACCOUNT", fg="yellow", bg="blue", font=("bold", 15),
                        command=login).place(x=310, y=580)

    submit = Button(frame1, text="Submit", fg="white", bg="grey", font=('Arial', 15), command=fr).place(x=400, y=420)
    b1 = Button(frame1, text="CREATE A NEW ACCOUNT", fg="yellow", bg="blue", font=('Arial', 15), command=fun).place(
        x=320, y=470)
